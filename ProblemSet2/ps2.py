    ...
    def __init__(self, width, height):
        ...
        for h in range(self.height):
            for w in range(self.width):
                #initialize a room with all dirty = all not clean = all False
                self.room[(w,h)] = False
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        # For each Trial, Create room and robot List
        room = RectangularRoom(width,height)
        robotList = []
    # Create robots and add them to the robotList
        for n in range(num_robots):
            robotList.append(robot_type(room,speed))
            #print robotList
        #anim = ps2_visualize.RobotVisualization(num_robots, width, height)
        steps = 0
    # While the room is not cleaned enough: Let all robots have another 'turn'
    ...
    # Add steps to stepsList
       ...
       #anim.done()
    # return mean-value of all values in stepsList
    return float(sum(stepsList)) / len(stepsList)

...
class Cluster(object):
    ...
    def averageLinkageDist(self, other):
        ...
        for point in self.points:
            for other_point in other.points:
                numComparations += 1
                totalDistance += point.distance(other_point)
        if numComparations > 0:
            return totalDistance / float(numComparations)
        else:
            return 0.0
    ...
    def findClosest(self, linkage):
        ...
        for c1 in self.members:
            for c2 in self.members:
                if c1 == c2:
                    continue
                linkageDistance = linkage(c1,c2)
                if linkageDistance < closestDist:
                    closestDist = linkageDistance
                    tuple = (c1, c2)
        return tuple
